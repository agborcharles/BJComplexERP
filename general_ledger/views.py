...
def general_ledger_view(request):
    ...
    transactions = Expenses.objects.all()


    #if start_date and end_date:
        #customer =  list(Expenses.objects.filter(created_at__range=[start_date, end_date]).aggregate(total=Count('invoice_id')).values())[0]
    ...
    #else:
        #customer =  list(Expenses.objects.aggregate(total=Count('invoice_id')).values())[0]
...
def transaction_details(request, slug):
    trans = get_object_or_404(Expenses, slug=slug)
